...
#       feature_lists["nombre"][idx] for idx in sampled_indices
#   ]
#   sampled_ratings = [
...
#     example_lists_by_user[user_id]["nombre"].append(
#         example["nombre"])
#     example_lists_by_user[user_id]["rating"].append(
#         example["rating"])
#     movie_title_vocab.add(example["nombre"].numpy())

#   tensor_slices = {"user_id": [], "nombre": [], "rating": []}
...
#       tensor_slices["nombre"].append(sampled_movie_titles)
#       tensor_slices["rating"].append(sampled_ratings)
...
#   sampled_indices = random_state.choice(
#       range(len(next(iter(feature_lists.values())))),
#       size=num_examples_per_list,
#       replace=False,
#   )

#   return {
#       feature: tf.stack([tf.reshape(feature_lists[feature][idx], [-1]) for idx in sampled_indices], 0)
...
#   tensor_slices = {feature: [] for feature in features}
#   tensor_slices["user_id"] = []

#   for user_id, feature_lists in example_lists_by_user.items():
#     for _ in range(num_list_per_user):

#       # Drop the user if they don't have enough ratings.
#       if len(next(iter(feature_lists.values()))) < num_examples_per_list:
#         continue
...
#       )
#       tensor_slices["user_id"].append(user_id)
#       for feature, value in sampled_features.items():
#         tensor_slices[feature].append(value)
